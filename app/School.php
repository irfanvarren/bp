...
class School extends Model
{
  ...
  /*  return dd($this->hasManyThrough(
    ...
    public function student_course_details($student_id){
        $course_details = CourseDetail::selectRaw('student_course_details.*,school_programs.name as program_name,school_programs.duration as program_duration,school_programs.fee as tuition_fee')->join('school_programs','school_programs.id','student_course_details.program_id')->where('student_course_details.school_id',$this->id)->where('student_course_details.student_id',$student_id)->orderBy('end_date','ASC')->get();
