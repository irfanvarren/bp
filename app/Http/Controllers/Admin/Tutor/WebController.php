...
class WebController extends Controller
{
    ...
    $current_test_id = $id;
    $test_result = OtCurrentTest::selectRaw('ot_current_tests.*,ot_registrations.*')->leftJoin('students','students.id','ot_current_tests.student_id')->leftJoin('ot_registrations','ot_registrations.id','ot_current_tests.registration_id')->find($id);
...
//    $myAnswer = OtCurrentAnswer::where('current_test_id',$current_test_id)->get();
    ...
    //dd($myAnswer);
    return view('online-test.check-answer',['myAnswer' => $myAnswer,'answer_structure' => $testAnswer,'test_id' => $test_id,'user_data' => $test_result]);*/
    }
    ...
    public function test_results(Request $request){
        ...
        //$test_results = OtCurrentTest::selectRaw('ot_current_tests.*,ot_tests.name as test_name,ot_modules.name as module_name,ot_packages.name as package_name,ot_registrations.*')->join('ot_tests','ot_tests.id','ot_current_tests.test_id')->join('ot_modules','ot_modules.id','ot_current_tests.module_id')->join('ot_packages','ot_packages.id','ot_current_tests.package_id')->join('ot_registrations','ot_registrations.id','ot_current_tests.registration_id')->orderBy('ot_current_tests.id','Desc')->paginate(10);
...
public function finish_test($id){
    ...
    $correct_answer = collect(['test_id' => $test_id,'module_id' => $module_id,'package_id_id' => $package_id]);
            ...
            foreach($section->structures as $keyStructure => $structure){
                ...
                foreach($structure->questions as $keyQuestion => $question){
                    ...
                    if($structure->option_type != "essay"){
                        $multiple_answer = OtTestAnswer::where('question_id',$question->id)->first()->answers;
