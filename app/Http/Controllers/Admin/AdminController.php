...
class AdminController extends Controller
{
  ...
  public function index(Request $request){
   ...
   $route = "admin";
    ...
    return view('admin.berdayakan.dashboard',['dashboard' => $dashboard,'default_route' => $route,'calendar_events' => $calendar_events,'events' => $events,'rules' => $rules,'enquiries' => $enquiries,'sk' => $sk,'contracts' => $contracts,'sk_karyawan' => $sk_karyawan,'contracts_karyawan' => $contracts_karyawan ,'job_description' => $job_description,'briefings' => $briefings,'berdayakan_calendar_events' => $berdayakan_calendar_events,'_menu_template' => 'Admin Berdayakan']);
  }
  ...
  public function document_translate(){
    $document_translations = RegisDocTranslate::get();
    return view('admin.document-translate',compact('document_translations'));
  }
...
//      $regisenglish = $regisenglish->select('tb_calonsiswa.*','tb_paket_bimbel.NAMA as nama_course','tb_level.NAMA as nama_level','tb_kategori.NAMA as kelompok_kelas')->join('tb_paket_bimbel','tb_paket_bimbel.KD','=','tb_calonsiswa.id_course')->join('tb_level','tb_calonsiswa.id_level','=','tb_level.KD')->join('tb_kategori','tb_calonsiswa.id_kelompok_kelas','=','tb_kategori.KD');
  ...
  public function online_test_registration(){
    $ot_registration = OtRegistration::selectRaw('ot_registrations.*,ot_tests.name as test_name,ot_modules.name as module_name')->orderBy('ot_registrations.id','DESC')->leftJoin('ot_tests','ot_registrations.test_id','ot_tests.id')->leftJoin('ot_modules','ot_modules.id','ot_registrations.module_id')->get();
  ...
  public function online_test_registration_details($id){
    $detail = OtRegistration::selectRaw('ot_registrations.*,tb_calonsiswa.*,ot_tests.name as test_name,ot_modules.name as module_name')->leftJoin('tb_calonsiswa','tb_calonsiswa.KD','ot_registrations.id_calonsiswa')->leftJoin('ot_tests','ot_registrations.test_id','ot_tests.id')->leftJoin('ot_modules','ot_modules.id','ot_registrations.module_id')->find($id);
    

    return view('admin.detail_online_test_registration',compact('detail'));
  }
  ...
  public function approve_online_test_registration(Request $request){
    if($request->status === "true"){
      ...
      $find_registration->update($request->except('id'));
      ...
      if(!$cek_current_test){
        ...
        $create_current_test = OtCurrentTest::where('registration_id',$request->id)->update($request->except('_token','status','id'));
      }
      ...
      Mail::send(new OnlineTestLink($token,$email,$test_name,$test_id));
        ...
        $update_status = OtRegistration::find($request->id)->update($request->except('id'));
      }
...
public function post_enquiry_details($id, Request $request){
  ...
  $dom = new \domdocument();
      ...
      public function island_ajax(Request $request){
        $islands = Island::where('REF_NEGARA',strtoupper($request->REF_NEGARA))->get();
        return response()->json($islands,200);  
      }
      public function province_ajax(Request $request){
       $provinces = Province::where('REF_PULAU',$request->REF_PULAU)->get();
       return response()->json($provinces,200);  
     }
     public function city_ajax(Request $request){
       $cities = City::where('REF_PROVINSI',strtoupper($request->REF_PROVINSI))->get();
       return response()->json($cities,200);  
     }
     public function district_ajax(Request $request){
       $districts = District::where('REF_KOTA',$request->REF_KOTA)->get();
       return response()->json($districts,200);  
     }
     public function sub_district_ajax(Request $request){
       $sub_districts = SubDistrict::where('REF_KECAMATAN',$request->REF_KEC)->get();
       return response()->json($sub_districts,200);  
     }
     public function recruitments(){
      $recruitments = Recruitment::orderBy('created_at','DESC')->get();
      return view('admin.recruitments',compact('recruitments'));
     }
          public function recruitments_detail($id){
      $recruitment = Recruitment::find($id);
      return view('admin.recruitments-detail',compact('recruitment'));
     }
   }
