...
class ResellerController extends Controller
{
	...
	public function index(){
		...
		$online_toefl_registrations = OtRegistration::selectRaw('ot_registrations.*,ot_tests.name as test_name,ot_modules.name as module_name,ot_current_tests.score,ot_current_tests.id as current_test_id,ot_current_tests.answers')->join('ot_tests','ot_registrations.test_id','ot_tests.id')->leftJoin('ot_current_tests','ot_current_tests.registration_id','ot_registrations.id')->leftJoin('ot_modules','ot_modules.id','ot_registrations.module_id')->where('reseller_id',$reseller->id)->where('ot_registrations.test_id',2)->orderBy('ot_registrations.id','DESC')->get();

		$online_ielts_registrations = OtRegistration::selectRaw('ot_registrations.*,ot_tests.name as test_name,ot_modules.name as module_name,ot_current_tests.score,ot_current_tests.id as current_test_id,ot_current_tests.answers')->join('ot_tests','ot_registrations.test_id','ot_tests.id')->leftJoin('ot_current_tests','ot_current_tests.registration_id','ot_registrations.id')->leftJoin('ot_modules','ot_modules.id','ot_registrations.module_id')->where('reseller_id',$reseller->id)->where('ot_registrations.test_id',1)->orderBy('ot_registrations.id','DESC')->get();

		$online_bept_registrations = OtRegistration::selectRaw('ot_registrations.*,ot_tests.name as test_name,ot_modules.name as module_name,ot_current_tests.score,ot_current_tests.id as current_test_id,ot_current_tests.answers')->join('ot_tests','ot_registrations.test_id','ot_tests.id')->leftJoin('ot_current_tests','ot_current_tests.registration_id','ot_registrations.id')->leftJoin('ot_modules','ot_modules.id','ot_registrations.module_id')->where('reseller_id',$reseller->id)->where('ot_registrations.test_id',3)->orderBy('ot_registrations.id','DESC')->get();
		
		$message = "";
		if($reseller->status == 0){
			$message = 'Akun reseller telah dinonaktifkan / masih belum diaktifkan oleh admin !';
		}
		$quotas = ResellerQuota::selectRaw('reseller_quotas.*,tb_paket_bimbel.NAMA as NAMA_PAKET')->with(['registration_data' => function($query){
		...
		return view('reseller.index',compact('languages','products','quotas','reseller','message','online_toefl_registrations','online_ielts_registrations','online_bept_registrations'));
	}
