...
class WebController extends Controller
{
    ...
    public function index()
    {
      ...
      $promo_news = PromoNews::orderBy('id','DESC')->limit(3)->get();
      $footer = "";
  ...
  if($reseller_id != ""){
    ...
    if($cart_exist){
      ...
      foreach($items as $item){
        ...
        if(array_key_exists($request->REF_PAKET.$request->REF_LEVEL,  $request->TGL_TEST)){
          $TGL_TEST = $request->TGL_TEST[$request->REF_PAKET.$request->REF_LEVEL];
...
public function registration_payment($kode = null,$slug = null){
  ...
  $cart_data = Cart::where('user_id',Auth::user()->id)->where('status',0)->with(['items' => function($query){
    return $query->selectRaw('cart_items.id,cart_items.cart_id,tb_paket_bimbel_dtlharga.REF_PAKET, tb_paket_bimbel_dtlharga.REF_LEVEL,tb_paket_bimbel_dtlharga.REF_KATEGORI,tb_paket_bimbel.REF_BIDANGUSAHA,tb_paket_bimbel.NAMA as NAMA_PAKET,tb_level.NAMA as NAMA_LEVEL,tb_kategori.NAMA as NAMA_KATEGORI,tb_paket_bimbel_dtlharga.HARGA_PAKET,cart_items.qty')->join('tb_paket_bimbel_dtlharga',function($join){
      ...
      return $query2->selectRaw('DATE_FORMAT(tgl_mulai, "%d-%c-%Y") as tgl_mulai,course_events.REF_PAKET,course_events.REF_LEVEL')->where('tgl_mulai','>=',date('Y-m-d',Carbon::now()->timestamp))->orderBy('tgl_mulai','ASC')->limit(2)->join('tb_events','tb_events.event_type_id','course_events.REF_EVENT_TYPE')->pluck('tgl_mulai'); 
    }])->get();
  }])->first();
  $jadwal = false;
  if(!$cart_data){
...
return view('news' ,['recent_news'=> $model2 ,'news' => $model->where('status',1)->where('type_id',1)->orderBy('id','Desc')->paginate(6),'this_tags' => $this_tags,'title' => 'news','news_types' => $news_types]);
}
...
public function media_promo(PromoNews $model){
 return view('promo' ,['news' => $model->where('type_id',1)->where('status',1)->orderBy('end_date','ASC')->paginate(20),'type' => "promo"]);
}
...
public function post_events_guest_book($kd,Request $request){
  ...
  //$events->REF_EVENT = $request->event;
    ...
    {
      $testimony = new Testimony;
      return view('testimony',['testimony' => $testimony->where('status',1)->get()]);
    }
  ...
  {
    return view('/study abroad/index');
  }
...
public function toefl_upload_payments_receipt(Request $request){
  ...
  $tgl_daftar = $request->tgl_daftar;
  $user_id = Auth::user()->id;
  $data = RegisPaket::where('users_id','=',$user_id)->where('_token','=',$token)->select('kd','created_at')->first();
  $kd = $data->kd;
  $tgl = $data->created_at;
  ...
  Mail::send(new UploadPaymentsReceiptMail($request));
...
{
  $companies = Perusahaan::get();
  return view('/career-application',compact('companies'));
}
public function post_career_application(Request $request){
  ...
  $path_ijazah = Storage::disk('public')->putFileAs($path,$ijazah,'ijazah'.$key.'-'.time().'.'.$ijazah->getClientOriginalExtension());
...
/*    public function post_ielts_simulation_school(Request $request){
    ...
    {
      return view('translate-document');
    }
...
public function select_category(Request $request){
  ...
  $pricelist_id = $request->pricelist_id;
  $category_id = $request->category_id;
  $duration= PacketPriceDetails::where('tb_paket_bimbel_dtlharga.KD',$pricelist_id)->where('tb_paket_bimbel_dtlharga.REF_LEVEL','=',$level_id)->where('tb_paket_bimbel_dtlharga.REF_PAKET','=',$course_id)->where('tb_paket_bimbel_dtlharga.REF_KATEGORI','=',$category_id)->select(DB::raw('CAST(tb_paket_bimbel_dtlharga.JUMLAH_JAM as integer) as JUMLAH_JAM'),DB::raw('CAST(tb_paket_bimbel_dtlharga.JUMLAH_PERTEMUAN as integer) as JUMLAH_PERTEMUAN'))->get();
    ...
    */
        ...
        public function admin(Request $request){
          ...
          if(auth()->user()->hasRole('Admin') || auth()->user()->hasRole('Super Admin') || auth()->user()->hasRole('Staff')){
            ...
            return view('dashboard',['dashboard' => $dashboard,'default_route' => $route,'calendar_events' => $calendar_events,'events' => $events,'rules' => $rules,'enquiries' => $enquiries,'sk' => $sk,'contracts' => $contracts,'sk_karyawan' => $sk_karyawan,'contracts_karyawan' => $contracts_karyawan ,'job_description' => $job_description,'briefings' => $briefings]);
          }
