...
class WebController extends Controller
{
   ...
   public function student_detail_profile_ajax(Request $request){
       ...
       $personal_information = PersonalInformation::selectRaw('student_personal_informations.*,visa_countries.name as visa_country_name,students.client_id')->leftJoin('countries as visa_countries','visa_countries.id','student_personal_informations.visa_country')->join('students','students.id','student_personal_informations.student_id')->join(DB::raw('(select * from student_visa_histories order by created_at DESC limit 1) as student_visa_histories'),'student_visa_histories.student_id','student_personal_informations.student_id')->where('student_personal_informations.student_id',$request->student_id)->with('student')->first();
...
public function personal_information_ajax(Request $request){
    ...
    $request->merge(['student_id' => $student_id]);
    $personal_information = PersonalInformation::where('student_id',$student_id)->first();
    $output = "Data has been updated successfully";
...
public function start_test(Request $request){
    ...
    //$path = 'online-test/'.date("Y").'/'.date("m").'/'.date("d").'/'.$student_id;
...
public function test(){
        /*$questions = OtQuestion::with('options')->limit('1')->offset('1')->first();
        ini buat nanti pas 1 soal 1 jak yg muncul
        */   
        //$question_num = $this->question_num; 
        //$duration = OtTest::find($this->test_id)->value('duration');
        
        //$this->timeout = $this->start_time + $duration; // waktu selesai
        //$timer_interval = $this->timeout - $this->start_time;
       // $introduction = $this->questions->first()->sections->where('type','introduction')->first()->structures->first();
        //dd(session('online_test'));

        /*
        $current_structure = $this->current_structure;
        $current_structure->test_section = 1;
        $current_structure->section = 2;
        $current_structure->structure = 24;
        session()->put("online_test.current_structure",$current_structure);
        */
        
        //dd($this->show_current_structure());
        ...
        //$test_section_duration = $test_section->duration;
        //$shuffledQuestions = $this->shuffledQuestions->slice($question_num - 1,1)->first();
        //$total = $this->shuffledQuestions->count(); 
        //session()->put('online_test.total_question',$total);
        //$current_test_qna = OtCurrentTestQna::where('current_test_id',$this->current_test_id)->where('question_id',$this->current_question_id)->first();
        //return view('online-test.test',['time'=> $this->start_time,'interval' => $timer_interval,'timeout' => $this->timeout,'shuffledQuestions' => $shuffledQuestions,'question_num' => $question_num,'total' => $total,'current_test_qna' => $current_test_qna]);
...
public function next_question(Request $request){
    if($request->question_type == "single"){
        ...
        if($request->answers != ""){
            ...
            $path = 'new-online-test/'.date("Y").'/'.date("m").'/'.date("d").'/'.$current_test_id.'/';
            $json_answers = Storage::disk('public')->exists($path.'answers.json') ? json_decode(Storage::disk('public')->get($path.'answers.json')) : [];
            dd($json_answers);
            $inputData = array('current_test_id' => $current_test_id,'test_id' => $this->test_id,'module_id' => $this->module_id,'package_id' => $this->package_id, 'test_section_id' => $this->show_current_test_section()->id,'section_id' => $this->show_current_section()->id,'question_id' => $this->show_current_structure()->questions->first()->id,'answer_id' => $answer_id,'answers' => $answers);
            array_push($json_answers,$inputData);
            Storage::disk('public')->put($path.'answers.json', json_encode($json_answers));
            */
...
public function finish_test(){
    ...
    $count = 0;
    $correct_answer = collect(['test_id' => $this->test_structure[0]->test_id,'module_id' => $this->test_structure[0]->module_id,'package_id_id' => $this->test_structure[0]->package_id,'test_section_id' => $this->test_structure[0]->id]);
