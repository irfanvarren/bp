...
class LoginController extends Controller
{
    ...
    */
    ...
    if (auth()->attempt(array($fieldType => $request->username, 'password' => $request->password),true)) {
      ...
      if(\URL::previous() == $route || $route == url('register') || $route == url('login') || $route == url('student/login') || $route == url('student/register') || $route == url('admin/login')){
        if(Auth::guard()->user()->hasRole('Admin') || Auth::guard()->user()->hasRole('Super Admin') || Auth::guard()->user()->hasRole('Staff')){
          return redirect('admin');
        }
        return redirect('home')->withMessage('Login Successful');
