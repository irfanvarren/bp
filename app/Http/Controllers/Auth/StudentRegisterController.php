...
class StudentRegisterController extends Controller
{
    ...
    */
    ...
    public function register(Request $request)
    {
     ...
     $numbering = str_pad($student_number,5,'0',STR_PAD_LEFT);
        //$client_id = "BP-".$numbering.'/'.$request->visa_country;

     $cek_student = User::where('email',$request->email)->get();
     if(!count($cek_student)){
      ...
      $input_data_user = $input_data_user->merge(['email' => $request->email]);
      ...
      $insert_user =  User::where('email',$request->email)->first();
      $user_id = $insert_user->id;
    }
    ...
    $input_data_student = $input_data_student->merge(['email' => $request->email]);
  ...
  /*$input_data_student = $input_data_student->merge(['client_id' => $client_id]);*/
    ...
    $input_data_personal_information = $input_data_personal_information->merge(['primary_email' => $request->email]);
