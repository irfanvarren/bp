  ...
  if($request->details != "")
  {
   $dom = new \domdocument();
    ...
    public function post_copy_programs(Request $request){
      ...
      if($request->from_program != ""){
        $arr_program_id = explode("|",$request->from_program);
        $school_programs = SchoolProgram::where('school_id',$request->from_school)->whereIn('id',$arr_program_id);
