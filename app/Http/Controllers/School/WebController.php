...
class WebController extends Controller
{
  ...
  public function __construct(){
    $ch = curl_init(); 
    curl_setopt($ch, CURLOPT_URL, "https://api.exchangerate.host/latest?base=IDR"); 
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); 

    $ex_rate_idr = curl_exec($ch);
    curl_close($ch);

    $this->ex_rate_idr =(array)json_decode($ex_rate_idr);
    if($this->ex_rate_idr['success']){
      $this->ex_rate_idr = $this->ex_rate_idr['rates'];
    }
    
    $ch2 = curl_init(); 
    curl_setopt($ch2, CURLOPT_URL, "https://api.exchangerate.host/latest?base=USD"); 
    curl_setopt($ch2, CURLOPT_RETURNTRANSFER, 1); 

    $ex_rate_usd = curl_exec($ch2);
    curl_close($ch2);
    $this->ex_rate_usd = (array)json_decode($ex_rate_usd);
    if($this->ex_rate_usd['success']){
      $this->ex_rate_usd = $this->ex_rate_usd['rates'];
    }
  }
  ...
  function new_search_school(Request $request){
    ...
    return view('sekolah.new.search',['qualifications' => $qualifications,'country' => $countries,'result' => $result,'searchResult' => $schools]);
  }
