    ...
    <div class="col-md-7 card text-center" style="margin-top:-25px;padding:30px">
        ...
        <div class="col-md-12 score-result">
            ...
            <table class="table table-bordered">
                ...
                <tr>    
                    <th>{{$value['name']}}</th> <td>{{$value['correct_answer'].' / '.$value['total_questions']}}</td><td class="text-center">@if($value['name']=='Writing') Not Yet Graded @else @if($value['score'] != 0) {{number_format($value['score'],1)}}	 @else  0 @endif @endif</td>
                </tr>
                @endforeach
                </tbody>
            </table> 
            ...
            -->
            <div class="col-md-12" style="text-decoration: underline;"> <h1> 
          
            IELTS Score : @if(fmod($result['total_score'],0.5 ) == 0) {{number_format(($result['total_score']-2)/2,1)}} @else {{number_format(round(($result['total_score'] - 2)/2,0	),1)}}  @endif </h1> </div>
            </div>
        </div>
          <div class="col-md-12 text-left" style="font-size:18px;">
                Alasan Mengikuti Test : {{$registration_data->ALASAN}}
