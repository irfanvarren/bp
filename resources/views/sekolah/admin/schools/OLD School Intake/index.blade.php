...
<table class="table">
    <tbody class="pendidikan-wrapper">
        ...
        <td rowspan="3" class="text-center"> <button type="button" rel="tooltip" title="" id="school_intake-btn" onclick="add_school_intake(this)" class="btn btn-primary btn-link btn-sm btn-add" value="add"
          data-original-title="Tambah">
          <i class="material-icons">add</i> <span>Add</span>
      </button></td>
  </tr>
  <tr>
    ...
    <td>
      ...
      <select class="selectize col-sm-12 pl-0 pr-0" required data-style="select-with-transition" id="school_intake_program" name="school_intake_program">
        <option value="">- Select Program -</option>
...
function open_school_intake_programs(){
    var token = $("input[name='_token']").val();
    $.ajax({
        url: "{{route('school_program-ajax')}}",
        method: "POST",
        data: {
            _token: token,
            school_id: school_id,
        ...
        success: function(data) {
            //$('#school_intake_program').html(data);
            //$('#school_intake_program').refreshOptions();
            var data = JSON.parse(data);
            var $select = $('#school_intake_program').selectize();
...
function add_school_intake(el){
    ...
    var program_id = $('#school_intake_program').val();
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            //$('#school_intake_program').val('');
            //$('#school_intake_program').selectpicker('refresh');
            var $select = $('#school_intake_program').selectize();
            var selectize = $select[0].selectize;
            selectize.setValue('');
            $('#school_intake-output').html(data);
...
function edit_school_intake(id,term_id,intake_date,orientation_date,note) {
