...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{route('school-region.update',$region)}}" enctype="multipart/form-data" autocomplete="off" class="form-horizontal">
            @csrf
            @method('put')
            <div class="card ">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-name" type="text" autocomplete="off" placeholder="{{ __('Region Name (ISO_3166-2)') }}" value="{{ old('name',$region->name) }}"  aria-required="true"/>
                      @if ($errors->has('name'))
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                        <div class="form-group{{ $errors->has('code') ? ' has-danger' : '' }}">
                          <input class="form-control{{ $errors->has('code') ? ' is-invalid' : '' }}" name="code" id="input-code" type="text" autocomplete="off" placeholder="{{ __('Region Code (ISO_3166-2:country_code)') }}" value="{{ old('code',$region->code) }}" aria-required="true"/>
                          @if ($errors->has('code'))
                    ...
                    <div class="form-group{{ $errors->has('currency_code') ? ' has-danger' : '' }}">
                      <div class="dropdown bootstrap-select col-sm-12 pl-0 pr-0">
                        ...
                        <select class="selectpicker col-sm-12 pl-0 pr-0" data-style="select-with-transition" id="country_id" required name="country_id">
                          ...
                          <option value="{{$value->id}}" {{$region->country_id == $value->id ? "selected" : ""}}> {{$value->name}}</option>
                          @endforeach
                        </select>
