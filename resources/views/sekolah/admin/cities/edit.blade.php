...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{route('school-city.update',$city)}}" enctype="multipart/form-data" autocoschool-citylete="off" class="form-horizontal">
            @csrf
            @method('put')
            <div class="card ">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-name" type="text" placeholder="{{ __('City Name') }}" value="{{ old('name',$city->name) }}"  aria-required="true"/>
                      @if ($errors->has('name'))
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                        <div class="form-group{{ $errors->has('latitude') ? ' has-danger' : '' }}">
                          <input class="form-control{{ $errors->has('latitude') ? ' is-invalid' : '' }}" name="latitude" id="input-latitude" type="text" placeholder="{{ __('Latitude') }}" value="{{ old('latitude',$city->latitude) }}" aria-required="true"/>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                        <div class="form-group{{ $errors->has('longitude') ? ' has-danger' : '' }}">
                          <input class="form-control{{ $errors->has('longitude') ? ' is-invalid' : '' }}" name="longitude" id="input-longitude" type="text" placeholder="{{ __('Longitude') }}" value="{{ old('longitude',$city->longitude) }}" aria-required="true"/>
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('region_id') ? ' has-danger' : '' }}">
                      <div class="dropdown bootstrap-select col-sm-12 pl-0 pr-0">
                        ...
                        <select id="region_id" name="region_id" style="width:100%;">

<!--                        <select class="selectpicker col-sm-12 pl-0 pr-0 " data-style="select-with-transition" id="region_id" name="region_id">-->
                          <option value="">- Select Region -</option>

                          @foreach($regions as $key => $value)
                          <option value="{{$value->id}}" {{ $value->id === $city->region_id ? "selected" : ""  }}> {{$value->name}}</option>
                          @endforeach
                        </select>
              ...
              <div class="card-footer ml-auto mr-auto">
                <button type="submit" class="btn btn-primary">{{ __('Edit city') }}</button>
              </div>
            </div>
          </form>
        </div>
