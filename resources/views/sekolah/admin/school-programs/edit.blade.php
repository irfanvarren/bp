...
<div class="content">
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
        <form method="post" action="{{route('school-program.update',$schoolProgram)}}" enctype="multipart/form-data" autocoschool-programlete="off" class="form-horizontal">
          @csrf
          @method('put')
          <div class="card ">
            <div class="card-header card-header-primary">
              <h4 class="card-title">{{ __('Update Program') }}</h4>
              ...
              <div class="row">
                ...
                <div class="col-md-7">
                  ...
                  <select required class="selectize form-control" id="schools" name="schools">
                    ...
                    <option value="{{$value->id}}" {{$schoolProgram->school_id == $value->id ? "selected" : ""}}> {{$value->name}}</option>
                    @endforeach
                  </select>

                  @if ($errors->has('school'))
                  <span id="harga-error" class="error text-danger" for="input-school">{{ $errors->first('school') }}</span>
                  @endif
                </div>
              </div>
            </div>
              ...
              <div class="col-md-7">
               <div class="form-group{{ $errors->has('courses') ? ' has-danger' : '' }}">
                ...
                <select required class="selectize form-control" id="courses" name="courses">
                  ...
                  <option value="{{$value->id}}" {{$schoolProgram->course_id == $value->id ? "selected" : ""}}> {{$value->name}}</option>
                  @endforeach
                </select>
            ...
            <div class="col-md-7">
             <div class="form-group{{ $errors->has('Sub Courses') ? ' has-danger' : '' }}">
              ...
              <select required class="selectize form-control" id="qualifications" name="qualifications">
                ...
                <option value="{{$value->id}}" {{$schoolProgram->qualification_id == $value->id ? "selected" : ""}}> {{$value->name}}</option>
                @endforeach
              </select>
          ...
          <div class="col-md-7">
           <div class="form-group{{ $errors->has('Sub Courses') ? ' has-danger' : '' }}">
            ...
            <select required class="selectize form-control" id="sectors" name="sectors">
              ...
              <option value="{{$value->id}}" {{$schoolProgram->sector_id == $value->id ? "selected" : ""}}> {{$value->name}}</option>
              @endforeach
            </select>
      ...
      <div class="row">
        ...
        <div class="col-sm-7">
          <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
            <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="name" type="text" placeholder="{{ __('Name') }}" value="{{ old('name',$schoolProgram->name) }}" aria-required="true"/>
            @if ($errors->has('name'))
      ...
      <div class="row">
        ...
        <div class="col-sm-7">
          <div class="form-group{{ $errors->has('material_fee') ? ' has-danger' : '' }}">
            <input class="form-control{{ $errors->has('material_fee') ? ' is-invalid' : '' }}" name="material_fee" id="input-material_fee" type="material_fee" placeholder="{{ __('Material Fee (Total)') }}" value="{{old('material_fee',$schoolProgram->material_fee) }}" aria-required="true"/>
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group{{ $errors->has('fee') ? ' has-danger' : '' }}">
                <input class="form-control{{ $errors->has('fee') ? ' is-invalid' : '' }}" name="fee" id="input-fee" type="text" placeholder="{{ __('Course Fee (Total)') }}" value="{{ old('fee',$schoolProgram->fee) }}" aria-required="true"/>
                <div>Ex: 1000</div>
                @if ($errors->has('fee'))
            ...
            <div class="col-md-6">
              <div class="form-group{{ $errors->has('fee_installment') ? ' has-danger' : '' }}">
                <input class="form-control{{ $errors->has('fee_installment') ? ' is-invalid' : '' }}" name="fee_installment" id="input-fee_installment" type="number" placeholder="{{ __('Course Fee installment') }}" value="{{ old('fee_installment',$schoolProgram->fee_installment) }}" aria-required="true"/>
                <div>Ex: 10 </div>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="form-group{{ $errors->has('duration') ? ' has-danger' : '' }}">
                <input class="form-control{{ $errors->has('duration') ? ' is-invalid' : '' }}" name="duration" id="input-duration" type="text" placeholder="{{ __('Duration') }}" value="{{ old('duration',$schoolProgram->duration) }}" aria-required="true"/> 
      ...
      <div class="row">
        ...
        <div class="col-sm-7">
          <div class="form-group{{ $errors->has('course_information') ? ' has-danger' : '' }}">
           <textarea name="course_information" class="form-control" rows="4" id="course_information">{!!$schoolProgram->course_information!!}</textarea>
