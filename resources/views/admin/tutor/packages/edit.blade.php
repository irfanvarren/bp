...
<div class="content">
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
        <form method="post" action="{{route('ot-package.update',$packages)}}" enctype="multipart/form-data" autocomplete="off" class="form-horizontal">
          @csrf
          @method('put')
          <div class="card ">
              ...
              <div class="row">
                ...
                <div class="col-sm-8">
                  <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                    <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-name" type="text" placeholder="{{ __('Name') }}" value="{{ old('name',$packages->name) }}"  aria-required="true"/>
                    @if ($errors->has('name'))
                ...
                <div class="col-sm-8">
                  <div class="form-group">
                     <select class="selectpicker col-sm-12 pl-0 pr-0" data-style="select-with-transition" name="test_id" id="test_id">
                                ...
                                <option value="{{$data_test->id}}" {{$packages->test_id == $data_test->id ? 'selected' : ''}}>{{$data_test->name}}</option>
                                @endforeach
                            </select>
                  </div>
                </div>
  ...
  <option value="">Without Module</option>
            ...
            <div class="card-footer ml-auto mr-auto">
              <button type="submit" class="btn btn-primary">{{ __('Edit package') }}</button>
            </div>
          </div>
        </form>
      </div>
