...
<div class="content">
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
        <form method="post" action="{{route('admin-briefing.update',$briefing)}}" enctype="multipart/form-data" autocomplete="off" class="form-horizontal" onsubmit="event.preventDefault();return validateMyForm(this);">
          @csrf
          @method('put')
          <div class="card ">
            <div class="card-header card-header-primary">
              <h4 class="card-title">{{ __('Edit Briefing Minutes') }}</h4>
            ...
            <div class="card-body ">
              ...
              <label for="" class="col-sm-2 col-form-label">{{__('Reference')}}</label>
              <div class="col-sm-7">
                <div class="form-group">
                  <input type="text" name="reference" id="reference" class="form-control" placeholder="Type" value="{{$briefing->reference}}">
                </div>
              </div>
            </div>
             <div class="row form-group">
              ...
              <div class="row form-group">
                ...
                <div class="col-sm-7">
                  <div class="form-group">
                    ...
                    <div>
                      <input type="checkbox" id="all-attendees" name="all-attendees" onclick="check_all_employees(this)" {{count($briefing->attendees) ? 'checked' : ''}}> All Employees
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div class="row">
                    <div class="col-sm-6">
                      <div class="form-group{{ $errors->has('date') ? ' has-danger' : '' }}">
                        <input class="form-control{{ $errors->has('date') ? ' is-invalid' : '' }} " name="date" id="input-date" type="date" value="{{ old('date',$briefing->date) }}" />
                      </div>
                    </div>
                    <div class="col-sm-6">
                     <div class="form-group{{ $errors->has('time') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('time') ? ' is-invalid' : '' }} timepicker" name="time" id="input-time" value='{{date("H:i",strtotime($briefing->time))}}' type="text" />
                      @if ($errors->has('time'))
            ...
            <div class="row form-group">
              ...
              <div class="col-sm-7">
                <div class="form-group">
                  <input type="text" name="type" id="type" class="form-control" placeholder="Type" value="{{$briefing->type}}">
                </div>
              </div>
            </div>
            <div class="row form-group">
              ...
              <div class="col-sm-7">
                <div class="form-group{{ $errors->has('announcement') ? ' has-danger' : '' }}">
                  <textarea name="announcement" id="announcement" class="form-control tinymce-editor"  rows="5">{{$briefing->announcement}}</textarea>
                </div>
              </div>
            </div>
            ...
            <div class="row form-group">
              ...
              <div class="col-sm-7">
                <div class="form-group{{ $errors->has('discussion') ? ' has-danger' : '' }}">
                 <textarea name="discussion" id="discussion" class="form-control tinymce-editor"  rows="5">{{$briefing->discussion}}</textarea>
               </div>
             </div>
           </div>
           <div class="row form-group">
            ...
            <div class="col-sm-7">
              <div class="form-group{{ $errors->has('conclusion') ? ' has-danger' : '' }}">
               <textarea name="conclusion" id="conclusion" class="form-control tinymce-editor"  rows="5">{{$briefing->conclusion}}</textarea>
             </div>
           </div>
         </div>
         <div class="row form-group">
          ...
          <div class="col-sm-7">
            <select class="selectpicker col-sm-12 pl-0 pr-0" data-style="select-with-transition" id="note_taker" name="note_taker">
              ...
              <option value="{{$employee->ID_KARYAWAN}}" {{$briefing->note_taker == $employee->ID_KARYAWAN ? 'selected' : ''}}>{{$employee->NAMA}}</option>
              @endforeach
            </select>
          </div>
        </div>
      ...
      <div class="card-footer ml-auto mr-auto">
        <button type="submit" class="btn btn-primary">{{ __('Edit Minutes') }}</button>
      </div>
    </div>
  </form>
</div>
...
<script type="text/javascript">
  ...
  $('#attendees').selectize({
    ...
    searchField: 'NAMA',
    options:employees,
    items:{!!$briefing->attendees != "" ? $briefing->attendees : "[]"!!},
    create: false,
  });
  $(function () {
