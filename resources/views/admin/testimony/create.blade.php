@extends('layouts.app-auth', ['activePage' => 'testimony','activeMenu' => 'data-management', 'namePage' => __('testimony')])
...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{route('admin-testimony.store')}}" enctype="multipart/form-data" autocomplete="off" class="form-horizontal">
            @csrf
            @method('post')
            <div class="card ">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('video') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('video') ? ' is-invalid' : '' }}" name="video" id="input-name" type="text" placeholder="{{ __('name') }}" value="{{ old('name') }}" />
                      @if ($errors->has('name'))
                        <span id="harga-error" class="error text-danger" for="input-name">{{ $errors->first('harga') }}</span>
                      @endif
                    </div>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-9">
                    <div class="form-group{{ $errors->has('testimony') ? ' has-danger' : '' }}">
                      <textarea class="form-control {{ $errors->has('testimony') ? ' is-invalid' : '' }}" name="testimony" id="input-testimony" type="text" placeholder="{{ __('testimony...') }}" value="{{ old('testimony') }}" row="5" required></textarea>
