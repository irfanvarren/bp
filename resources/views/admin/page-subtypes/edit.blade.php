...
<div class="content">
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
        <form method="post" action="{{route('admin-page-subtype.update',['division' => $division,$subtype])}}" enctype="multipart/form-data" autocomplete="off" class="form-horizontal">
          @csrf
          @method('put')
          <div class="card ">
            <div class="card-header card-header-primary">
              <h4 class="card-title">{{ __('Edit Sub Type') }}</h4>
            ...
            <div class="card-body ">
              ...
              <div class="row">
                <label class="col-sm-2 col-form-label">{{ __('Sub Type') }}</label>
                <div class="col-sm-7">
                  <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                    <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-title" type="text" placeholder="{{ __('Sub Type') }}" value="{{ old('name',$subtype->name) }}" required />
                    @if ($errors->has('name'))
            ...
            <div class="card-footer ml-auto mr-auto">
              <button type="submit" class="btn btn-primary">{{ __('Edit Sub Type') }}</button>
            </div>
          </div>
        </form>
      </div>
