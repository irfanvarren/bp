...
<div class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <form method="post" action="{{ route('admin-event-types.update',$event_type)}}" autocomplete="off" class="form-horizontal">
                    @csrf
                    @method('put')
                     <div class="card ">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                        <input class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" id="input-name" type="text" placeholder="{{ __('Type') }}" value="{{ old('name',$event_type->name) }}" required />
                                        @if ($errors->has('name'))
                        ...
                        <div class="card-footer ml-auto mr-auto">
                            <button type="submit" class="btn btn-primary">{{ __('Edit Event Type') }}</button>
                        </div>
                    </div>
                </form>
            </div>
