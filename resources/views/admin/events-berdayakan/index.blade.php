          ...
          <div class="card-body">
            ...
            <div class="row">
              <div class="col-12 text-right">
                <a href="{{ route('admin-events-berdayakan.create') }}" class="btn btn-sm btn-primary">{{ __('Add Events') }}</a>
              </div>
            </div>
            <div class="row">
              <div class="col-md-12">
                ...
                <a class="btn {{$type_id == $events_type->id ? 'btn-primary' : 'btn-warning'}} event-btn" href="{{url('admin/events-berdayakan/event-type/'.$events_type->id)}}">{{$events_type->name}}</a>
                @endforeach
              </div>
            </div>
        ...
        function change_status(label,id,evt){
          ...
          if(evt.checked == true){
            ...
            if(confirm("Are you sure want to change this status ?")){
             ...
             $(label).html("Active");
             $.ajax({
              url: "{{route('admin-events-berdayakan.change_status')}}",
              method: 'POST',
              data: {
                _token: token,
                id:id,
