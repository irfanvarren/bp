...
<div class="content">
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
        <form method="post" action="{{ route('admin-events-berdayakan.update',$id)}}" enctype="multipart/form-data" autocomplete="off" class="form-horizontal">
          @csrf
          @method('put')
          <div class="card ">
            <div class="card-header card-header-primary">
              <h4 class="card-title">{{ __('Edit Events') }}</h4>
              <p class="card-category"></p>
            </div>
            <div class="card-body ">
              <div class="row">
                <div class="col-md-12 text-right">
                  <a href="{{route('admin-events-berdayakan.index') }}" class="btn btn-sm btn-primary">{{ __('Back to list') }}</a>
                </div>
              </div>
              ...
              <div class="row">
                ...
                <div class="col-sm-7">
                  <select class="selectpicker col-sm-12 pl-0 pr-0" data-style="select-with-transition" id="event_type_id" name="event_type_id">
                    ...
                    <option value="{{$value->id}}" {{$value->id == $events->event_type_id ? 'selected' : ''}}> {{$value->name}}</option>
                    @endforeach
                  </select>
                </div>
              </div> 
              <div class="row">
                ...
                <div class="col-sm-2">
                  <div class="form-check-inline">
                    <input class="form-check-input" type="checkbox" name="full_day_event" id="full_day_event" {{$events->full_day_event ? 'checked' : ''}}> full day event ?
                  </div>

                </div>
              </div>
              <div id="test-module-wrapper" style="display:{{$events->event_type_id == 2 ? 'block' : 'none'}}">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('test_module') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('test_module') ? ' is-invalid' : '' }}" name="test_module" placeholder="Test Module" id="input-test_module" type="text" value="{{$events->test_module}}" />
              ...
              <div class="row">
                ...
                <div class="col-sm-7">
                  <div class="form-group{{ $errors->has('jam_mulai') ? ' has-danger' : '' }}">
                    <input class="form-control timepicker {{ $errors->has('jam_mulai') ? ' is-invalid' : '' }}" name="jam_mulai" id="input-jam_mulai" value='{{date("H:i",strtotime($events->jam_mulai))}}' type="time" />
              ...
              <div class="row">
                ...
                <div class="col-sm-7">
                  <div class="form-group{{ $errors->has('jam_selesai') ? ' has-danger' : '' }}">
                    <input class="form-control timepicker {{ $errors->has('jam_selesai') ? ' is-invalid' : '' }}" name="jam_selesai" id="input-jam_selesai" value='{{date("H:i",strtotime($events->jam_selesai))}}' type="time" />
              ...
              <div class="row" id="webinars-wrapper" style=" @if($events->event_type_id != 6)display:none;@endif">
                <div class="col-sm-12">
                  <hr>
                </div>
                <div class="col-sm-12">
                   ...
                   <div class="col-sm-7 col-form-label text-left">
                    <div class="togglebutton" >
                      <label id="status-ajax">
                        ...
                        <input type="checkbox" id="status" class="status_input" @if($articles->status) checked  @else @endif  name="status">
                        <span class="toggle"></span>
                      </label>

                      <span id="status-label" class="status-label" for="status">
                        @if($articles->status) Active @else Non Active @endif</span>
                      </div>
                    </div>
                  </div>
                  <div class="row">
                    ...
                    <div class="col-sm-7">
                      <div class="{{ $errors->has('gambar') ? ' has-danger' : '' }}">
                        <div class="fileinput fileinput-new" data-provides="fileinput">
                          <div class="fileinput-preview img-thumbnail" data-trigger="fileinput" style="width: 200px; height: 150px;"><img src="{{Storage::disk('events')->url($articles->image)}}"></div>
                          <div>
                  ...
                  <div class="row">
                    ...
                    <div class="col-sm-7">
                      <div class="form-group{{ $errors->has('image_desc') ? ' has-danger' : '' }}">
                        <input class="form-control{{ $errors->has('image_desc') ? ' is-invalid' : '' }}" name="image_desc" id="input-image_desc" type="text" placeholder="{{ __('Deskripsi Gambar') }}" value="{{ old('image_desc',$articles->image_desc) }}" />
                    ...
                    <div class="row">
                      <div class="col-sm-12">
                        <div class="form-group{{ $errors->has('body') ? ' has-danger' : '' }}">
                          <textarea class="form-control tinymce-editor {{ $errors->has('body') ? ' is-invalid' : '' }}" name="news_body" id="input-body" type="text" placeholder="{{ __('Content...') }}" value="{{ old('body') }}" row="3">{{$articles->body}}</textarea>
                          @if ($errors->has('body'))
                      ...
                      <div class="col-sm-12">
                        <div style="float:left;">
                         <input class="form-control{{ $errors->has('version_date') ? ' is-invalid' : '' }}" name="version_date" id="input-version_date" type="date" placeholder="{{ __('Version Date') }}" value="{{ old('version_date',$articles->version_date) }}" />
                       </div>
                       <div style="float:right;">
                         <input class="form-control{{ $errors->has('version') ? ' is-invalid' : '' }}" name="version" id="input-version" type="text" placeholder="{{ __('Version') }}" value="{{ old('version',$articles->version) }}" />
                       </div>
                     </div>
                   </div>
                 </div>
         ...
         <div class="card-footer ml-auto mr-auto">
          <button type="submit" id="submit-btn" class="btn btn-primary">{{ __('Edit Events') }}</button>
        </div>
      </div>
    </form>
  </div>
...
<script>
  ...
  $(document).ready(function(){
   if($('#full_day_event').prop("checked") == true){
    $('#input-tgl_selesai').prop("disabled",true);
    $('#input-jam_mulai').prop("disabled",true);
    $('#input-jam_selesai').prop("disabled",true);
