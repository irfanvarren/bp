...
<div class="content">
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
        <form method="post" action="{{route('admin-page-content.update',$pageContent)}}" enctype="multipart/form-data" autocomplete="off" class="form-horizontal">
          @csrf
          @method('put')
          <div class="card ">
            <div class="card-header card-header-primary">
              <h4 class="card-title">{{ __('Edit Page Content') }}</h4>
                ...
                <div class="col-sm-8">
                  <div class="form-group">
                   <select class="selectpicker col-sm-12 pl-0 pr-0" data-style="select-with-transition" name="content_type" id="content_type">
                    ...
                    <option value="{{$value->id}}" {{ $pageContent->content_type == $value->id ? 'selected' : ''}}>{{$value->name}}</option>
                    @endforeach
                  </select>
                </div>
              </div>
            ...
            <div class="row">
              ...
              <div class="col-sm-8">
                 <textarea class="form-control tinymce-editor{{ $errors->has('content') ? ' is-invalid' : '' }}" name="content" id="content" placeholder="{{ __('content') }}" value="{{ old('content') }}" row="3">{{$pageContent->content}}</textarea>
          ...
          <div class="card-footer ml-auto mr-auto">
            <button type="submit" class="btn btn-primary">{{ __('Edit Page Content') }}</button>
          </div>
        </div>
      </form>
    </div>
