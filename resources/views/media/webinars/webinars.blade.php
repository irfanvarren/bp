...
<div class="content-wrapper col-md-12" style="padding:30px;">
 ...
 <!--   <div class="col-md-3">-->
    ...
    @if(strtotime($data_news->tgl_mulai) >= \Carbon\Carbon::now()->timestamp)
    <a href="{{url("media/events/".$type."/".date("Y/m",strtotime($data_news->created_at)).'/'.$data_news->slug)}}" style="color:black">
      <div class="item">
        <div class="animate-box">
          <div class="image-popup fh5co-board-img" title="{{$data_news->title}}"><img src="{{ Storage::disk('events')->url($data_news->image) }}" alt="{{$data_news->title}}"></div>
        </div>
        <div class="fh5co-desc">
          <div style="padding: 15px 0">{{$data_news->title}}</div>
          @if(strtotime($data_news->tgl_mulai) >= \Carbon\Carbon::now()->timestamp)
          <div style="background:#2E62A6;color:white;width:100%;padding:7px;text-align:center;width: 100%;" id="timer-wrapper{{$key}}">
    ...
    @endif
    @php
    array_push($timer_list,strtotime($data_news->tgl_mulai));
    @endphp
    @endforeach
  </div>
  <div class="col-md-12">
